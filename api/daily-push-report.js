...
function buildDailySummary(userTransactions = [], dateString = '', timeLabel = '') {
  let pemasukan = 0;
  let pengeluaran = 0;
...
${expenseLine}`,
    url: '/dashboard.html',
    tag: `olah-uang-daily-${dateString}-${String(timeLabel || '').replace(/[^0-9]/g, '')}`
  };
}
...
export default async function handler(req, res) {
  try {
    ...
    const timeLabel = getJakartaTimeLabel();
    ...
    for (const sub of subscriptions || []) {
      const payload = buildDailySummary(groupedTransactions[sub.user_id] || [], range.dateString, timeLabel);
    ...
    return res.status(200).json({ ok: true, date: range.dateString,
      time: timeLabel, subscriptions: subscriptions?.length || 0, transactions: transactions?.length || 0, sent, failed });
  } catch (error) {
