...
window.db = db;
const APP_VERSION = '20260908-v133-dark-top';
...
// ============================================================
const OLAH_SWAL_DEFAULTS = {
  customClass: {
    popup: 'olah-swal-popup',
    title: 'olah-swal-title',
    htmlContainer: 'olah-swal-html',
    actions: 'olah-swal-actions',
    confirmButton: 'olah-swal-confirm',
    cancelButton: 'olah-swal-cancel',
    closeButton: 'olah-swal-close'
  },
  buttonsStyling: false,
  reverseButtons: true
};

function swalConfig(config = {}) {
  const rawConfirmText = String(config.confirmButtonText || '').toLowerCase();
  const isDangerConfirm = /hapus|keluar|delete|logout|trash|suspend|nonaktif/.test(rawConfirmText);

  const incomingClass = config.customClass || {};
  const baseConfirmClass = [
    OLAH_SWAL_DEFAULTS.customClass.confirmButton,
    incomingClass.confirmButton || '',
    isDangerConfirm ? 'olah-swal-danger-confirm' : ''
  ].filter(Boolean).join(' ');

  const customClass = {
    ...OLAH_SWAL_DEFAULTS.customClass,
    ...incomingClass,
    confirmButton: baseConfirmClass
  };
...
function applyUnifiedPopupDefaults() {
  if (typeof Swal !== 'undefined' && typeof Swal.mixin === 'function') {
    window.AppSwal = Swal.mixin(OLAH_SWAL_DEFAULTS);
  }
}

applyUnifiedPopupDefaults();

function patchSwalFireForUnifiedDesign() {
  if (typeof Swal === 'undefined' || Swal.__olahUnifiedPatched) return;

  const originalFire = Swal.fire.bind(Swal);

  Swal.fire = function patchedOlahSwalFire(...args) {
    if (args.length === 1 && args[0] && typeof args[0] === 'object' && !Array.isArray(args[0])) {
      return originalFire(swalConfig(args[0]));
    }

    return originalFire(...args);
  };

  Swal.__olahUnifiedPatched = true;
}

patchSwalFireForUnifiedDesign();
...
function updateAppPwaThemeColor(enabled = isDarkMode()) {
  ...
  ['theme-color', 'msapplication-navbutton-color'].forEach((name) => {
    let metas = document.querySelectorAll(`meta[name="${name}"]`);
    if (!metas.length) {
...
function renderInsightList(insights, { source = 'static', cached = false } = {}) {
  ...
  const statusClass = aiInsightLoading
    ? (dark ? 'bg-slate-500 border-emerald-900 text-emerald-200' : 'bg-emerald-50 border-emerald-100 text-emerald-700')
    : hasError
      ? (dark ? 'bg-slate-500 border-rose-900 text-rose-200' : 'bg-rose-50 border-rose-100 text-rose-700')
      : isAI
        ? (dark ? 'bg-slate-500 border-emerald-900 text-emerald-200' : 'bg-emerald-50 border-emerald-100 text-emerald-700')
        : (dark ? 'bg-slate-500 border-slate-800 text-slate-400' : 'bg-gray-50 border-gray-100 text-gray-400');
    ...
    <div class="space-y-3">
      ${safeInsights.map((text, index) => {
        const cardClass = isAI
          ? (dark
            ? 'bg-slate-500 border-emerald-900 shadow-lg'
            : 'bg-gradient-to-br from-emerald-50 to-teal-50 border-emerald-100 shadow-[0_10px_24px_rgba(5,150,105,.08)]')
          : (dark ? 'bg-slate-500 border-slate-800' : 'bg-gray-50 border-gray-100');
...
// PWA INSTALL PROMPT
// ============================================================
const PWA_PRODUCTION_ORIGIN = 'https://olahuang.vercel.app';
const PWA_PRODUCTION_INSTALL_URL = `${PWA_PRODUCTION_ORIGIN}/index.html`;

function isProductionAppOrigin() {
  return window.location.origin === PWA_PRODUCTION_ORIGIN;
}
...
async function updatePwaInstallButton() {
  ...
  buttons.forEach((btn) => {
    ...
    btn.title = isProductionAppOrigin()
      ? 'Install Olah Uang dari olahuang.vercel.app'
      : 'Buka olahuang.vercel.app untuk install aplikasi';
  });
}
...
async function installPwaApp() {
  ...
  if (!isProductionAppOrigin()) {
    return Swal.fire({
      ...
      html:
        'Untuk memasang versi aplikasi, buka dari domain resmi:<br><br>' +
        '<b>olahuang.vercel.app</b><br><br>' +
        'Testing dari localhost/VS Code tidak selalu memunculkan prompt install resmi. Jadi instalasi diarahkan ke link production, bukan dari alamat lokal.',
      showCancelButton: true,
      confirmButtonText: 'Buka Link Install',
      cancelButtonText: 'Batal',
      confirmButtonColor: '#059669'
    }).then((result) => {
      if (result.isConfirmed) {
        window.open(PWA_PRODUCTION_INSTALL_URL, '_blank', 'noopener,noreferrer');
      }
    });
  }
...
async function syncDailyPushStatus() {
  ...
  try {
    ...
    if (subscription && Notification.permission === 'granted') return setDailyPushUiState('active', 'Status: aktif 22.00 WIB');
...
async function enableDailyPushNotification() {
  ...
  try {
    ...
    if (existingSubscription) {
      ...
      await syncDailyPushStatus();
      return Swal.fire({ icon: 'success', title: 'Notifikasi sudah aktif', text: 'Ringkasan harian akan dikirim pukul 22.00 WIB.', timer: 1600, showConfirmButton: false });
    }
    ...
    return Swal.fire({ icon: 'success', title: 'Notifikasi aktif', text: 'Ringkasan harian akan dikirim pukul 22.00 WIB.', timer: 1600, showConfirmButton: false });
  } catch (error) {
