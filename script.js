...
const APP_VERSION = '20260619-v47-domi-ai-persona';
...
function renderDomiAvatar(index = 1, { compact = false } = {}) {
  const sizeClass = compact ? 'h-9 w-9 rounded-2xl text-xl' : 'h-11 w-11 rounded-[1.35rem] text-2xl';
  const badgeSize = compact ? 'h-4 min-w-4 text-[8px]' : 'h-5 min-w-5 text-[9px]';

  return `
    <span class="relative ${sizeClass} shrink-0 overflow-visible flex items-center justify-center bg-gradient-to-br from-emerald-300 via-teal-500 to-emerald-800 text-white shadow-[0_12px_26px_rgba(5,150,105,.26)] ring-1 ring-white/60" title="Domi AI" aria-label="Domi AI">
      <span class="absolute inset-[3px] rounded-[inherit] bg-gradient-to-br from-white/30 via-transparent to-black/20 pointer-events-none"></span>
      <span class="absolute -top-1 -right-1 ${badgeSize} rounded-full bg-white text-emerald-700 border border-emerald-100 shadow-md font-extrabold flex items-center justify-center px-1">AI</span>
      <span class="relative drop-shadow-[0_5px_8px_rgba(15,23,42,.22)]">🤖</span>
      <span class="absolute -bottom-1 -left-1 h-4 w-4 rounded-full bg-amber-300 border-2 border-white shadow-md flex items-center justify-center text-[9px]">💸</span>
    </span>`;
}
...
function renderInsightList(insights, { source = 'static', cached = false } = {}) {
  ...
  const statusText = aiInsightLoading
    ? 'Domi AI sedang mengolah ringkasan data bulan ini. Sabar, dia lagi menghitung dosa belanja dulu 😌'
    : isAI
      ? (cached ? 'Insight Domi AI tersimpan untuk data periode ini.' : 'Insight otomatis dibuat oleh Domi AI dari ringkasan data periode ini.')
      : hasError
        ? `Insight AI belum berhasil dibuat otomatis: ${aiInsightError}`
        : hasPeriodData
          ? 'Domi AI sedang disiapkan otomatis. Jika belum muncul, tunggu beberapa detik lalu refresh halaman.'
          : 'Insight AI akan dibuat otomatis saat data periode ini tersedia.';
  ...
  container.innerHTML = `
    <div class="space-y-3">
      ${safeInsights.map((text, index) => {
        const avatar = isAI
          ? renderDomiAvatar(index + 1)
          : `<span class="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white text-gray-500 border border-gray-100 text-[11px] font-extrabold">${index + 1}</span>`;
        ...
        const textClass = isAI ? 'text-emerald-950' : 'text-gray-700';
        return `
          <div class="flex gap-3 rounded-3xl ${cardClass} border p-4">
            ${avatar}
            <div class="min-w-0 flex-1">
              ${isAI ? `<p class="mb-1 text-[10px] font-extrabold uppercase tracking-[.18em] text-emerald-600">Domi AI #${index + 1}</p>` : ''}
              <p class="${textClass} leading-7 text-[15px] md:text-base">${escapeHTML(text)}</p>
            </div>
          </div>`;
      }).join('')}
    </div>
    <div class="mt-4 rounded-2xl ${statusClass} border px-3 py-2 text-[11px] leading-5">
