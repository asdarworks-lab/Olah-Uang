...
const APP_VERSION = '20260619-v44-ai-insight-status-fix';
...
function renderInsightList(insights, { source = 'static', cached = false } = {}) {
  ...
  const statusText = aiInsightLoading
    ? 'AI sedang menyiapkan insight otomatis dari ringkasan data bulan ini.'
    : isAI
      ? (cached ? 'Insight AI tersimpan untuk data periode ini.' : 'Insight otomatis dibuat dari ringkasan data periode ini.')
      : hasError
        ? `Insight AI belum berhasil dibuat otomatis: ${aiInsightError}`
        : hasPeriodData
          ? 'Insight AI sedang disiapkan otomatis. Jika belum muncul, tunggu beberapa detik lalu refresh halaman.'
  ...
  container.innerHTML = `
    <div class="space-y-3">
      ${safeInsights.map((text, index) => `
        <div class="flex gap-3 rounded-2xl ${isAI ? 'bg-emerald-50 border-emerald-100' : 'bg-gray-50 border-gray-100'} border p-3">
          <span class="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${isAI ? 'bg-emerald-600 text-white' : 'bg-white text-gray-500 border border-gray-100'} text-[11px] font-extrabold">${isAI ? 'AI' : index + 1}</span>
          <p class="${isAI ? 'text-emerald-900' : 'text-gray-700'} leading-6">${escapeHTML(text)}</p>
        </div>
      `).join('')}
    </div>
